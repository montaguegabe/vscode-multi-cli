...
def test_update_gitignore_with_generated_files_updates_root_and_subrepos(tmp_path):
    ...
    assert "CLAUDE.md" not in root_gitignore
    assert "AGENTS.md" not in root_gitignore
    ...
    assert "CLAUDE.md" not in repo_a_gitignore
    assert "AGENTS.md" not in repo_a_gitignore
    ...
    assert "CLAUDE.md" not in repo_b_gitignore
    assert "AGENTS.md" not in repo_b_gitignore
    assert ".vscode/settings.json" not in repo_b_gitignore


def test_update_gitignore_with_generated_files_includes_agent_outputs_when_enabled(
    tmp_path,
):
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    (repo_a / ".git").mkdir(parents=True)
    (repo_b / ".git").mkdir(parents=True)
    (repo_b / "AGENTS.parts").mkdir()
    (repo_b / "AGENTS.parts" / "base.md").write_text("Repo instructions\n")
